...
try:
    __config_file = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "windninjawrapper.config.yaml"
    )  # os.environ["WNSERVER_CONFIG"]
    print(__config_file)
    ...
    CONFIG.QUEUE_DIRECTORY = os.path.abspath(
        overrides["job_wrapper"]["queue_datastore"]
    )
    CONFIG.MAIL = overrides["mail"]
    CONFIG.AUTO_FORECASTS = overrides["job_wrapper"]["auto_forecasts"]
    CONFIG.DEM_MIN_BOUNDING_GEOM_WKB = overrides["job_wrapper"][
        "dem_min_bounding_geom_wkb"
    ]
    ...
    CONFIG.TILE_GRAB_FILE_TEMPLATE = overrides["job_wrapper"]["tile_grabber"][
        "file_template"
    ]
    ...
    CONFIG.TILE_RASTER_MAX_THREADS = overrides["job_wrapper"]["tile_maker"][
        "max_threads"
    ]
    ...
    MESSAGES.BBOX_OUTSIDE_FORECASTS = overrides["job_wrapper"]["messages"][
        "bbox_outside_forecasts"
    ]
