...
def main():
    ...
    parser.add_argument(
        "-p", "--pretty_print", action="store_true", help="Pretty print job file"
    )
    ...
    try:
        ...
        if project is None or project.job is None or project.error is not None:
            logging.error(
                "Exiting: Unable to open project file: {}".format(project.error)
            )
            project = None
        elif project.job["status"] != JobStatus.new.name:
            logging.error(
                "Exiting: Project is not NEW: status={}".format(project.job["status"])
            )
            project = None
        else:
            project.updateJob(
                JobStatus.executing.name,
                (logging.INFO, "Initializing WindNinja Run"),
                True,
            )
            ...
            if project.forecast.lower() == "auto":
                ...
                logging.debug(
                    "evaluated forecast for bbox: {}".format(evaluated_forecast)
                )
                if evaluated_forecast:
                    ...
                    project.updateJob(
                        ...
                        (
                            logging.INFO,
                            "Auto Forecast Evaluated: {}".format(evaluated_forecast),
                        ),
                        True,
                    )
                else:
            ...
            if result[0]:
                ...
                if result[0]:
                    project.updateJob(
                        None, (logging.INFO, "WindNinjaCLI executed"), True
                    )
                    ...
                    if project.products.get("weather", False):
                        converted_weather = processShapefiles(
                            results_folder,
                            result[5],
                            project.path,
                            True,
                            where="speed>0",
                            zip_name="wx_geojson.zip",
                        )
                        if converted_weather[0]:
                            project.updateJob(
                                None,
                                (logging.INFO, "Weather converted to geojson"),
                                True,
                            )
                            ...
                            output = project.output["weather"] = {
                                ...
                                "data": {"maxSpeed": {"overall": wx_max_speed}},
                            }
                            for i in wx_infos:
                            ...
                            project.updateJob(
                                None, (logging.ERROR, converted_weather[1]), True
                            )

                    # windninja resutls as geojson vectors
                    if project.products.get("vector", False):
                        converted_windninja = processShapefiles(
                            results_folder,
                            result[3],
                            project.path,
                            True,
                            zip_name="wn_geojson.zip",
                        )
                        if converted_windninja[0]:
                            project.updateJob(
                                None,
                                (logging.INFO, "Output converted to geojson"),
                                True,
                            )
                            ...
                            output = project.output["vector"] = {
                                ...
                                "data": {"maxSpeed": {"overall": wn_max_speed}},
                            }
                            for i in wn_infos:
                                name = i.replace("shp", "json")
                            ...
                            project.updateJob(
                                None, (logging.ERROR, converted_windninja[1]), True
                            )
                    ...
                    if project.products.get("topofire", False):
                        ...
                        topofire_zip_file = grab_tiles(
                            project.bbox, project.path, "topofire"
                        )

                        if topofire_zip_file:
                            project.updateJob(
                                None, (logging.INFO, "TopoFire tiles compiled"), True
                            )
                            ...
                            project.updateJob(
                                None,
                                (logging.WARNING, "TopoFire tiles unavailable"),
                                True,
                            )
                    ...
                    if project.products.get("raster", False):
                        ...
                        if not wn_infos:
                            converted = processShapefiles(
                                results_folder, wn_shpfiles, project.path, False
                            )
                        ...
                        max_speed = (
                            wn_max_speed
                            if (wn_max_speed > wx_max_speed)
                            else wx_max_speed
                        )

                        # NOTE: weather points NOT drawn in tiles, but max speed maybe from weather ....
                        # TODO: should this return an error/status?
                        tile_zip = make_tiles_for_output(
                            project.path,
                            (results_folder, wn_shpfiles),
                            (wn_infos, max_speed),
                            project.forecast,
                        )
                        project.updateJob(
                            None,
                            (logging.INFO, "Output converted to raster tiles"),
                            True,
                        )
                        ...
                        output = project.output["raster"] = {
                            ...
                            "data": {"maxSpeed": {"overall": max_speed}},
                        }
                    ...
                    if project.products.get("clustered", False):
                        ...
                        if not wn_infos:
                            ...
                            for f in [a for a in result[4] if a.find("vel") > 0]:
                                wn_infos[f] = getRasterInfo(
                                    os.path.join(results_folder, f)
                                )

                            wn_max_speed = sorted(
                                wn_infos.values(), key=lambda x: x["max"], reverse=True
                            )[0]["max"]
                        ...
                        file_format = "json"
                        clustered_file, breakdown = createClusters(
                            results_folder,
                            project.path,
                            "wn_clustered",
                            native_wkid,
                            separate=False,
                            given_max_vel=max_speed,
                            format=file_format,
                        )
                        project.updateJob(
                            None, (logging.INFO, "Output converted to cluster"), True
                        )

                        # TODO: zip file
                        zip_name = "wn_clustered.zip"
                        zip_files(
                            os.path.join(project.path, zip_name),
                            [os.path.join(project.path, f) for f in clustered_file],
                        )
...
def processShapefiles(
    results_folder, shpfiles, project_path, convertToGeoJson, where=None, zip_name=""
):
    zip_package = None
    geoJson = None
